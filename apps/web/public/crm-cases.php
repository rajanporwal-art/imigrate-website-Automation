...
if ($action === 'case-save') {
    ...
    $rec = [
        ...
        'status' => c_str($in['status'] ?? 'New Case', 60),
        'targetDate' => c_str($in['targetDate'] ?? '', 20),
        'fee' => c_str($in['fee'] ?? '', 40),
        'assessingAuthority' => c_str($in['assessingAuthority'] ?? '', 120),
        'stateNomination' => c_str($in['stateNomination'] ?? '', 60),
        'fileNumber' => c_str($in['fileNumber'] ?? '', 80),
        'reminderDate' => c_str($in['reminderDate'] ?? '', 20),
